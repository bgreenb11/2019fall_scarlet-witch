  ...
  <v-card width="350" height="auto" min-height="300" class="mx-auto my-8 inline-block">
    <v-container>
      ...
      <v-row no-gutters justify="space-between">
        <v-col align="center">
          <p
            v-if="bridges.length !== 0 || index === 0"
            class="subtitle-2 text-center text-uppercase"
          >{{this.pages[this.index]}}</p>
          <p
            v-if="bridges.length === 0 && index === 1"
            class="subtitle-2 text-center text-uppercase"
          >{{this.missing}}</p>
        </v-col>
      </v-row>
...
export default {
  data() {
    return {
      ...
      bridge_ip: null,
      missing:
        "We could not find a bridge on the network. Please enter your bridge's IP address below.",
      pages: [
